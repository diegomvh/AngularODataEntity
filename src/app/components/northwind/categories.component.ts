...
export class CategoriesComponent {
  ...
  constructor() {
    const schema = this.categoriesService.structuredTypeSchema;
    if (schema !== null) {
  ...
  filter(event: Event, field: string) {
    ...
    const resource = this.categoriesService.entities();
    if (value) {
      resource.query((q) => {
  ...
  loadCategoriesLazy(event: TableLazyLoadEvent) {
    const resource = this.categoriesService.entities().query((q) => {
