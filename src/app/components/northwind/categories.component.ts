...
@Component({
  ...
  template: `<p-table #table [columns]="cols" [value]="rows" [lazy]="true" (onLazyLoad)="loadCategoriesLazy($event)" [paginator]="true"
...
export class CategoriesComponent {
  ...
  loadCategoriesLazy(event: LazyLoadEvent) {
    ...
    if (event.sortField !== undefined)
      resource = resource.orderBy([[event.sortField as keyof Category, event.sortOrder == -1 ? "desc": "asc"]]);
    this.fetch(resource);
  }
}
